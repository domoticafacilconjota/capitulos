...
if (isset($_REQUEST["upload"])) {
	if ($_FILES['minimal_firmware']["name"] == "") {
		...
		try {
			...
			if ($_FILES['minimal_firmware']["type"] == "application/octet-stream"
				...
				$ext = "bin";
			} else if ($_FILES['minimal_firmware']["type"] == "application/x-gzip") {
                $ext = "bin.gz";
            }
			else {
				throw new RuntimeException(
	...
	try {
		...
		if ($_FILES['new_firmware']["type"] == "application/octet-stream"
			...
			$ext = "bin";
		} else if ( $_FILES['new_firmware']["type"] == "application/x-gzip") {
            $ext = "bin.gz";
...
elseif (isset($_REQUEST["auto"])) {
	...
	if ($fwAsset != "") {
		...
		foreach ($data->assets as $binfileData) {
			if ($binfileData->name == "tasmota-minimal.bin.gz") {
		...
		if (isset($fwUrl) && isset($fwMinimalUrl)) {
			$minimal_firmware_path = $firmwarefolder . 'tasmota-minimal.bin.gz';
			$new_firmware_path     = $firmwarefolder . 'tasmota.bin.gz';
